...
import { readSiteViewCount } from "@/lib/site-views/server";
...
async function checkPocketBaseLive() {
  try {
...
export async function buildSystemStatsPayload() {
  ...
  const [viewsResult, backendLive] = await Promise.all([
    readSiteViewCount(),
    checkPocketBaseLive(),
  ]);

  return {
    viewCount: viewsResult.ok ? viewsResult.count : null,
